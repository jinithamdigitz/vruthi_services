            ...
            <form action="{{ route('admin.timelines.update', $timeline) }}" method="POST">
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="number" class="form-control @error('year') is-invalid @enderror" id="year" name="year" 
                           min="1900" max="2100" value="{{ old('year', $timeline->year) }}" placeholder="e.g., 2007" required>
                    @error('year')
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" 
                           value="{{ old('title', $timeline->title) }}" placeholder="e.g., Company Established" required>
                    @error('title')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" class="form-control @error('description') is-invalid @enderror" id="description" 
                           name="description" value="{{ old('description', $timeline->description) }}" placeholder="e.g., Founded">
                    @error('description')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" class="form-control @error('icon') is-invalid @enderror" id="icon" name="icon" 
                           value="{{ old('icon', $timeline->icon) }}" placeholder="e.g., bi-rocket-takeoff" required>
